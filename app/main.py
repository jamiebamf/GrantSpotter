...
def execute_crawl_in_thread() -> None:
    ...
    try:
        ...
        crawl_state.update({
            ...
            "result": None, "error": str(exc)[:2000],
        })
...
def start_crawl_thread() -> dict:
    ...
    with crawl_lock:
        if crawl_thread and crawl_thread.is_alive():
            return {"status": "already_running", "started_at": crawl_state["started_at"]}
        started_at = datetime.now(timezone.utc).isoformat()
        crawl_state.update({
            "status": "starting", "started_at": started_at, "finished_at": None,
...
async def lifespan(app: FastAPI):
    scheduler.add_job(
        scheduled_crawl, "cron", hour=settings().schedule_hour_utc, minute=0,
        id="all-sources-daily", replace_existing=True,
    )
    scheduler.start()
    yield
...
def list_sources():
    result = Database().client.table("grant_sources").select(
        "id,source_name,slug,last_crawled_at,last_crawl_success,last_error"
    ).order("source_name").execute()
...
def review_grant(grant_id: str, action: ReviewAction):
    ...
    if action.action == "reject":
        return db.update_grant(grant_id, {
